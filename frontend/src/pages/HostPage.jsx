...
const HostPage = () => {
  // Refs for local/remote video, peer connection, and data channel
  const localVideo = useRef();
  const remoteVideo = useRef();
  const pc = useRef(null);
  const dataChannel = useRef(null);

  // State for offer, answer input, chat messages, and message input
  const [offer, setOffer] = useState('');
  const [answerInput, setAnswerInput] = useState('');
  const [chat, setChat] = useState([]);
  const [msg, setMsg] = useState('');

  // Called when "Create Offer" button is clicked
  const startConnection = async () => {
    // Create data channel for chat before offer
    ...
    // Create and set local offer, display it for copy-paste
    const offer = await pc.current.createOffer();
    await pc.current.setLocalDescription(offer);
    setOffer(JSON.stringify(offer));
  };

  // Called when "Set Answer" button is clicked
  const handleSetAnswer = async () => {
    try {
      // Parse and set remote answer from Joiner
  ...
  // Setup peer connection and local media on mount
  useEffect(() => {
    ...
    // When remote track is received, show it in remote video
    pc.current.ontrack = (e) => {
      remoteVideo.current.srcObject = e.streams[0];
    };
  }, []);

  // UI layout using react-bootstrap
  return (
    <Container className="mt-4">
      ...
      <Card className="mt-4">
        <Card.Body>
          ...
          {/* Button to create offer and show it for copy-paste */}
          ...
          {/* Button to set answer from Joiner */}
